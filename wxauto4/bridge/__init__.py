...
# 仅在 Windows 下导入 WeChat 并提供桥接启动；在其它平台（如 macOS）只暴露 WsClient/serializer
if sys.platform == 'win32':
    ...
    # 非 Windows 平台不提供 start_ws_bridge，以免误用
    def start_ws_bridge(*_, **__):  # type: ignore[override]
        raise RuntimeError('start_ws_bridge 仅在 Windows 平台可用；macOS 可直接使用 wxauto4.bridge.ws_client.WsClient')
